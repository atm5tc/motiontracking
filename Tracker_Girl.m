...
sizes = [96,128];               % Frame size
bt_space = [24,56];             % Location of image template at 30, 56
box = [36, 36];                 % Size of image template square(36)
framesize = 500;                % Number of frames
...
for f = 1:framesize
    stream(f,track(f,1):track(f,1)+box(2),[track(f,2) track(f,2)+box(1)]) = 255;
    stream(f,[track(f,1) track(f,1)+box(2)],track(f,2):track(f,2)+box(1)) = 255;
end
% Create AVI video and play using MATLAB Video Viewer
v = VideoWriter('Girl.avi');
open(v);
